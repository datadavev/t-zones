...
export class TZones extends LitElement {
  ...
  columnTimes(col) {
    ...
    return html`
      ...
      <clipboard-copy style="width:4rem; padding:0.4rem; border-radius:4px; border: 1px solid red; display: inline-block" for="times" class="button" @clipboard-copy="${this._copied}">Copy</clipboard-copy>
    </div>`;
  }
  ...
  render(){
      ...
      for (const rw of this.dmatrix) {
          ...
          for (let i=1; i < rw.length; i++) {
              ...
              row.push(html`<td class="${hv_class}" @click=${this.showTime} x-col="${i}" title="${rw[i].toFormat("dd-LLL HH:mm ZZ")}">${hv}</td>`)
          }
      ...
      return html`
          <vaadin-date-picker id="tz-date-picker" theme="small" value="${this.tnow.toFormat("yyyy-MM-dd")}"></vaadin-date-picker>
          <table>
              <tbody>
              ${rm}
              </tbody>
  ...
  firstUpdated() {
    ...
    picker.addEventListener('change', event => {
      ...
      _this.tnow = _this.tnow.set({year:_dt.year, month: _dt.month, day:_dt.day});
